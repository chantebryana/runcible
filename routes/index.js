...
// pull the dates logged in time_temp table and populate into array dates_logged:
						...
						function populate_y_axis_data(rows_from_db, a_match) {
							var y_temp_f = [];
							var count = 0;
						...
						function logged_time_taken(rows_from_db, a_match) {
							var x_time_taken = [];
							var count = 0;
							for (var i = 0; i < a_match.length; i++) {
								if (a_match[i] == 1) { // if match == true
...
*/
						...
						var y_temp_f = populate_y_axis_data( rows_from_db, comparison_key( auto_compute_date_range(), logged_dates() ) );
						var x_label_values = populate_x_axis_labels( auto_compute_date_range(dates_from_db), logged_time_taken( rows_from_db, comparison_key( auto_compute_date_range(), logged_dates() ) ) );
